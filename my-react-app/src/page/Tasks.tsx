...
function Tasks() {
    ...
    const fetchTasks = async () => {
        ...
        try {
            const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/tasks/team/${teamId}`, {
                headers: { 'Authorization': `Bearer ${token}` }
            });
    ...
    const handleCreateTask = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const payload = {
                ...
                assignedToUserIds: assignedToId ? [assignedToId] : []
            };
    ...
    const getAssignedNames = (task: Task): string => {
        if (!task.assignments || task.assignments.length === 0) return 'Unassigned';
        return task.assignments.map(a => a.user.name).join(', ');
    };
                    ...
                    <div className="bg-slate-800 border border-slate-700/50 rounded-xl p-8 w-full max-w-md shadow-2xl">
                        ...
                        <form onSubmit={handleCreateTask} className="flex flex-col space-y-5">
                            <div className='space-y-3'>
                                ...
                                <label className="block text-sm font-medium text-slate-300">Assign To (Optional)</label>
                                <select
                                    value={assignedToId}
